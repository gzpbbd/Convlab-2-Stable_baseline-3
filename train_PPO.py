...
def sampler(pid, queue, evt, env, policy, batchsz):
    ...
    while sampled_num < batchsz:
        ...
        for t in range(traj_len):
            ...
            if done:
                logging.debug('steps {}'.format(t * 2 + 2))  # todo
                break
...
def update(env, policy, batchsz, epoch, process_num):
    ...
    # logging.debug('sampling dialogs')
    tmp = time.time()
    ...
    logging.error('sample size {}, time {:.2f}'.format(batchsz_real, time.time() - tmp))
    # logging.debug('updating PPO')

    tmp = time.time()
    policy.update(epoch, batchsz_real, s, a, r, mask)
    logging.error('update time {:.2f}'.format(time.time() - tmp))
...
if __name__ == '__main__':
    ...
    init_logging_handler(args.log_dir, '_train_PPO')
