...
class Analyzer:
    ...
    def comprehensive_analyze(self, sys_agent, model_name, total_dialog=100):
        ...
        for j in tqdm(range(total_dialog), desc="dialogue"):
            ...
            usr_da_list = []  # 当前对话的所有 user action
            failed_da_sys = []
            failed_da_usr = []
            last_sys_da = None
                    ...
                    for da1 in sess.user_agent.get_out_da():
                        for da2 in sess.sys_agent.get_in_da():
                            if da1 != da2 and da1 is not None and da2 is not None and (da1, da2) not in failed_da_sys:
                                failed_da_sys.append((da1, da2))  # system NLU 分类错误的 action
                    ...
                    for da1 in last_sys_da:
                        for da2 in sess.user_agent.get_in_da():
                            if da1 != da2 and da1 is not None and da2 is not None and (da1, da2) not in failed_da_usr:
                                failed_da_usr.append((da1, da2))  # 表示 NLU 与 NLG 的错误

                last_sys_da = sess.sys_agent.get_out_da() if hasattr(sess.sys_agent, "get_out_da") else None
                usr_da_list.append(sess.user_agent.get_out_da())  # user NLU 分类错误的 action

                if session_over:
                    break

            logging.debug('dialog {} has {} steps'.format(j + 1, step))
            ...
            if (j + 1) % 100 == 0:
                ...
                logging.info("dialogue %d", j + 1)
                logging.info(sess.evaluator.goal)
                logging.info('task complete: %.3f', complete_num / (j + 1))
                logging.info('task success: %.3f', suc_num / (j + 1))
                logging.info('book rate: %.3f', np.mean(match))
                logging.info('inform precision/recall/f1: %.3f %.3f %.3f', np.mean(precision), np.mean(recall),
                            np.mean(f1))
            ...
            da_list = usr_da_list
            cycle_start = []  # 该轮对话中重复过的 user action
            ...
            domain_turn = []  # 该轮对话的每一轮中 user action 对应的 domain
            for da in usr_da_list:
            ...
            for domain in domain_set:
                ...
                if domain_success is not None:
                    reporter.record(domain, domain_success, sess.evaluator.domain_reqt_inform_analyze(domain),
        ...
        reporter.report(complete_num / total_dialog, suc_num / total_dialog, np.mean(precision), np.mean(recall),
                        np.mean(f1), tmp, turn_num / total_dialog)

        return complete_num / total_dialog, suc_num / total_dialog, np.mean(precision), np.mean(recall), np.mean(
            f1), np.mean(match), turn_num / total_dialog
    ...
    def compare_models(self, agent_list, model_name, total_dialog=100):
        ...
        for i in range(len(agent_list)):
            ...
            complete, suc, pre, rec, f1, match, turn = self.comprehensive_analyze(agent_list[i], model_name[i],
                                                                                  total_dialog)
            y0.append(complete)
            y1.append(suc)
            y2.append(pre)
