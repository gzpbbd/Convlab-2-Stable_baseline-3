...
class DomainRecorder():
    ...
    def format_result(self):
        ...
        for da in self.failed_nlu_da_sys:
            tmp = ['-'.join(da), self.failed_nlu_da_sys[da][0],
                   [('-'.join(fda), self.failed_nlu_da_sys[da][1][fda]) for fda in self.failed_nlu_da_sys[da][1]]]
            dalist_sys.append(tmp)

        dalist_usr = []
        for da in self.failed_nlu_da_usr:
            tmp = ['-'.join(da), self.failed_nlu_da_usr[da][0],
                   [('-'.join(fda), self.failed_nlu_da_usr[da][1][fda]) for fda in self.failed_nlu_da_usr[da][1]]]
...
class Reporter():
    ...
    def record(self, domain, suc, inform, fail1, fail2, cycle, turn):
        '''
        对于一通对话的统计数据，按 domain 保存数据到 self.recorders 中

        :param domain: str
        :param suc: 该 domain 是否成功
        :param inform: TP, FP, FN, bad_inform, reqt_not_inform, inform_not_reqt
        :param fail1: system NLU 分类错误的所有 action
        :param fail2: user NLU 分类错误的所有 action
        :param cycle: 该轮对话中重复出现过多次的所有 user action
        :param turn: # 该轮对话的每一轮中 user action 对应的 domain
        :return:
        '''
    ...
    def report(self, com, suc, pre, rec, f1, turn_suc, turn):
        ...
        :param com:
        :param suc: join success rate
        :param pre: join precise
        :param rec: join recall
        :param f1:  join F1
        :param turn_suc: 所有 succeed dialog 的 average turn
        :param turn: average turn
        :return:
        '''
        ...
        # frequency of domain 图 与 performance for each domain 图
        writer.write_png()
    ...
    def format_result(self):
        ...
        cols = ['Total Num', 'Succ Rate', 'Precision', 'Recall', 'F1', 'Dialog Loop Failed Rate', 'Dialog Turn (Succ)',
                'Dialog Turn (All)']
        table = []
