...
class Database(object):
    ...
    def query(self, domain, constraints, ignore_open=False, soft_contraints=(), fuzzy_match_ratio=60,
              focus_quantity=False):
        '''
        Returns the list of entities for a given domain based on the annotation of the belief state

        :param domain: str
        :param constraints: dict_items([(key1, value1), (key2, value2), ...])
        :param ignore_open:
        :param soft_contraints:
        :param fuzzy_match_ratio:
        :param focus_quantity:
        :return:
        '''
        # query the db
        if domain == 'taxi':
        ...
        for i, record in enumerate(self.dbs[domain]):
            ...
            # 如果 record 不满足 constrains, 就 break；如果 for 没有 break 过，就把 recode 加入 found 中
            for (key, val), fuzzy_match in chain(constraints_iterator, soft_contraints_iterator):
                if val == "" or val == "dont care" or val == 'not mentioned' or val == "don't care" or val == "dontcare" or val == "do n't care":
                    ...
                    try:
                        ...
                        if key.lower() not in record_keys:  # 记录中没有该 key
                            continue
                        if key == 'leaveAt':
