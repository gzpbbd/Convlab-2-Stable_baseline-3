...
class PPO(Policy):
    ...
    def predict_vec(self, state, state_vec):
        '''
        Predict an system action given state.

        :param state: dict 类型。在得到 action 后，state['system_action'] 会设置为该 action。Please refer to util/state.py
        :param state_vec: 一维的 numpy.ndarray
        :return: action: dict 类型。with the form of (act_type, {slot_name_1: value_1, slot_name_2, value_2, ...})
        '''
        s_vec = torch.Tensor(state_vec)
        a = self.policy.select_action(s_vec.to(device=DEVICE), False).cpu()
        action = self.vector.action_devectorize(a.numpy())
        state['system_action'] = action
        return action
    ...
    def update(self, epoch, batchsz, s, a, r, mask):
        ...
        for i in range(self.update_round):
            ...
            logging.debug(
                'training PPO, epoch {}, iteration {}, value_loss {}, policy_loss {}'.format(epoch, i, value_loss,
                                                                                             policy_loss))
    ...
    def load(self, filename):
        ...
        for value_mdl in value_mdl_candidates:
            if os.path.exists(value_mdl):
                ...
                logging.info('PPO: loaded checkpoint from file: {}'.format(value_mdl))
                break
        ...
        for policy_mdl in policy_mdl_candidates:
            if os.path.exists(policy_mdl):
                ...
                logging.info('PPO: loaded checkpoint from file: {}'.format(policy_mdl))
                break
