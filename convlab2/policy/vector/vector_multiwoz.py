...
class MultiWozVector(Vector):
    ...
    def pointer(self, turn):
        '''
        根据当前信息，查询 database，根据查询到的 entities 的数量，生成一维向量

        :param turn: state['belief_state']
        :return: 一维向量
        '''
        pointer_vector = np.zeros(6 * len(self.db_domains))
        for domain in self.db_domains:
            constraint = turn[domain.lower()]['semi'].items()
            entities = self.db.query(domain.lower(), constraint, focus_quantity=True)
    ...
    def state_vectorize(self, state):
        ...
        # degree
        degree = self.pointer(state['belief_state'])  # todo
