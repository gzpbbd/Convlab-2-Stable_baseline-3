...
class ActMLEPolicyDataLoader():
    ...
    def _build_data(self, root_dir, processed_dir):
        ...
        for part in ['train', 'val', 'test']:
            ...
            for belief_state, context_dialog_act, terminated, dialog_act in \
                    tqdm.tqdm(zip(raw_data['belief_state'], raw_data['context_dialog_act'], raw_data['terminated'],
                        raw_data['dialog_act'])):
                state = default_state()
        ...
        for part in ['train', 'val', 'test']:
            with open(os.path.join(processed_dir, '{}.pkl'.format(part)), 'wb') as f:
                ...
                logging.info('Preprocessing {} data done, saved in {}'.format(part,
    ...
    def create_dataset(self, part, batchsz):
        logging.debug('Start creating {} dataset'.format(part))
        s = []
        a = []
        ...
        logging.debug('Finish creating {} dataset'.format(part))
        return dataloader
