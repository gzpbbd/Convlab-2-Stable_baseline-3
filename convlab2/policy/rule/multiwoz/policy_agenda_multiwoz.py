...
class UserPolicyAgendaMultiWoz(Policy):
    ...
    def _normalize_value(cls, domain, intent, slot, value):
        ...
        if value not in value_list:
            ...
            logging.debug('Value not found in standard value set: value %s, slot: %s, domain: %s' % (value, slot, domain))
        return value
        ...
        for [slot, _] in slot_vals:
            if slot == 'time':
                if domain in ['train', 'restaurant']:
                    ...
                    logging.warning('illegal booking slot: %s, domain: %s' % (slot, domain))
                    continue
